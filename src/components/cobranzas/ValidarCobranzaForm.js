        ...
        <form onSubmit={handleSubmit} className="space-y-5" noValidate>
            <div className="flex items-start justify-between border-b border-slate-200 pb-4">
                <div>
                    ...
                    <h3 id="cobranza-form-title" className="mt-1 text-xl font-bold text-slate-950">Validar cobranza</h3>
                    <p className="mt-1 text-sm text-slate-500">{collection.numeroFactura || 'Sin factura asociada'} · {collection.cliente}</p>
                </div>
                <button type="button" onClick={onCancel} className="rounded-lg p-2 text-slate-400 transition hover:bg-slate-100 hover:text-slate-700 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-slate-200" aria-label="Cerrar formulario"><CloseIcon /></button>
            </div>
            ...
            <label htmlFor="montoValidado" className="block text-sm font-semibold text-slate-700">
                <span className="mb-2 block tracking-[0.01em]">Monto validado <span className="text-red-500">*</span></span>
                <input id="montoValidado" type="number" min="0.01" step="0.01" value={amount} onChange={(event) => { setAmount(event.target.value); setError(''); }} className="mt-2 w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-900 shadow-sm transition focus:border-blue-500 focus:outline-none focus-visible:ring-4 focus-visible:ring-blue-100 placeholder:text-slate-400" autoFocus aria-required="true" />
            </label>

            <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm" aria-live="polite">
            ...
            <div className="flex gap-3 border-t border-slate-200 pt-4">
                ...
                <button type="submit" className="flex-1 rounded-lg bg-blue-700 px-4 py-2.5 text-sm font-semibold text-white shadow-sm shadow-blue-700/20 transition hover:bg-blue-800 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue-200">Guardar validación</button>
            </div>
        </form>
    );
}
